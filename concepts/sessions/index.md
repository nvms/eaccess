...
1. Attaches it to `req.auth`
1. Calls `resyncSession()` to refresh session data from the database
1. Calls `processRememberDirective()` to restore sessions from remember-me cookies
...
The `ActivityLogger` records actions like login, failed login, 2FA prompts, remember token creation, role changes, and more. It parses user agent strings for browser/OS/device info and stores metadata as JSON. Activity logging is enabled by default. See the [Express Middleware](https://nvms.github.io/eaccess/guides/express-middleware/index.md) guide for configuration options.
